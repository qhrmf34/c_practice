...
fork_and_exec_worker(int serv_sock, int clnt_sock, int session_id, struct sockaddr_in *clnt_addr, ServerState *state)
{
    ...
    else if (pid == 0)                                                           /* 자식 프로세스 */
    {
        ...
        if (errno == ENOENT || errno == EACCES)                                                     /* 실행 파일 없음 */
            fprintf(stderr, "[자식 #%d] worker 실행파일 불가\n", session_id);
...
handle_child_died(ServerState *state)
{
    ...
    log_message(state->log_ctx, LOG_DEBUG, "좀비 회수: %d개, 남은 Worker: %d개", state->zombie_reaped, state->worker_count);
}
