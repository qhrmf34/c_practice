...
accept_client(int serv_sock, struct sockaddr_in *clnt_addr, ServerState *state)
{
    ...
    if (clnt_sock == -1)                                                         /* accept 실패 */
    {
        ...
        {
            log_message(state->log_ctx, LOG_DEBUG, "accept_client() : 재시도");
            return -1;
        }
...
run_server(void)
{
    ...
    while (state.running) 
    {
        ...
        if (pfd.revents & (POLLERR | POLLHUP | POLLNVAL))                        /* 소켓 에러 이벤트 */
        {
            log_message(&log_ctx, LOG_ERROR, "run server() : 서버 소켓 에러");                  /* POLLERR: 소켓 오류, POLLHUP: 연결 끊김, POLLNVAL: 잘못된 fd */
            continue;
        }
